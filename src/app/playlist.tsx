...
import { getUserCollectionState, loadPlaylistDetail, PlaylistDetail, toggleUserCollectionItem } from '@/services/music';
import { subscribeToLibraryRefresh } from '@/services/navigation-events';

export default function PlaylistDetailScreen() {
  const { id, owned, source, title } = useLocalSearchParams<{ id: string; owned?: string; source?: string; title?: string }>();
  ...
  const playlistSource = source === 'audius' || source === 'crimson' ? source : undefined;
  const collectionPlayback = useCollectionPlayback(detail?.songs ?? [], detail?.playlist.title ?? '', String(id));
  ...
  const renderHeaderPlayback = useCallback(() => (
    <CollectionHeaderPlayButton
      ...
      songs={detail?.songs ?? []}
    />
  ), [detail?.playlist.title, detail?.songs, id]);
  ...
  const reload = useCallback(async (showError: boolean) => {
    ...
    try {
      ...
      if (isOffline && offlineSongs.length && request === reloadRequest.current) {
      ...
      if (showError && request === reloadRequest.current) {
        Alert.alert('Playlist unavailable', 'Could not load this playlist.');
      }
    }
  }, [downloadsReady, id, isDownloaded, isOffline, isOwned, playlistSource, songsForCollection, title, uid]);
  ...
  useEffect(() => {
    ...
    if (!isOffline && !isOwned && user?.uid && id) getUserCollectionState(user.uid, 'LikedPlaylists', String(id))
      .then((value) => { if (request === likeRequest.current) setLiked(value); })
      .catch(() => undefined);
    return () => { likeRequest.current += 1; };
  }, [id, isOffline, isOwned, user?.uid]);

  if (!detail) return (
    <View style={[styles.loading, { backgroundColor: colors.background }]}>
      {loadError ? <>
        <Text style={[styles.empty, { color: colors.secondaryText }]}>This playlist could not be loaded. Check your connection and try again.</Text>
        <BouncyPressable accessibilityRole="button" accessibilityLabel="Retry loading playlist" onPress={() => void reload(false)}>
          <Text style={{ color: colors.accent, padding: 16 }}>Try again</Text>
        </BouncyPressable>
  ...
  const toggleLike = async () => {
    ...
    const request = ++likeRequest.current;
    try {
      const value = await toggleUserCollectionItem(user.uid, 'LikedPlaylists', playlist.id, playlist);
      if (request === likeRequest.current) setLiked(value);
    }
    catch { Alert.alert('Could not update playlist', 'Please try again.'); }
    finally { likePending.current = false; }
  };
  const openSongActions = (song: PlaylistDetail['songs'][number]) => router.push(actionSheetHref({ type: 'song', id: song.id, title: song.title, subtitle: song.creator, image: song.imageSmall || song.image, artistId: song.artistId }));
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <FlatList
        ...
        renderItem={({ item: song }) => (
          <View style={{ paddingHorizontal: 12 }}>
            <DetailSongRow
              ...
              song={song}
              unavailableForOffline={downloads.isTrackUnavailableForCollection(downloadKey, song.id)}
              onPress={() => playSong(song, songs, playlist.title, playlist.id)}
              onLongPress={() => openSongActions(song)}
            />
          </View>
        )}
        ListEmptyComponent={<Text style={[styles.empty, { color: colors.secondaryText }]}>This playlist doesn&apos;t have any songs yet.</Text>}
        ...
        <View style={styles.hero}>
          <PlaylistCover borderRadius={0} playlist={playlist} preferLarge showPlayingIndicator={false} style={StyleSheet.absoluteFill} />
          <LinearGradient colors={['transparent', 'rgba(14,13,19,0.62)', colors.background]} style={StyleSheet.absoluteFill} />
          <View style={styles.heroCopy}>
            <Text style={styles.name}>{playlist.title}</Text>
            <Text style={styles.metadata}>{isOwned ? playlist.artists : `${playlist.likes} likes · ${playlist.artists}`}</Text>
            <View style={styles.actions}>
              {!isOwned ? <BouncyPressable accessibilityRole="button" accessibilityLabel={liked ? 'Remove playlist from library' : 'Save playlist to library'} onPress={() => void toggleLike()} style={[styles.iconButton, { backgroundColor: colors.controlSurface, borderColor: liked ? colors.accent : colors.border }]}><SymbolView name={liked ? 'heart.fill' : 'heart'} size={21} tintColor={liked ? colors.accent : colors.text} /></BouncyPressable> : null}
              <BouncyPressable
                accessibilityLabel={allDownloaded ? `${playlist.title} is available offline` : `Make ${playlist.title} available offline`}
                disabled={!songs.length || downloading || allDownloaded}
                onPress={() => void savePlaylistOffline()}
                style={[styles.iconButton, { backgroundColor: colors.controlSurface, borderColor: allDownloaded ? colors.accent : colors.border }]}>
                {downloading
              ...
              <BouncyPressable
                accessibilityLabel={collectionPlayback.playing ? `Pause ${playlist.title}` : `Play ${playlist.title}`}
...
const styles = StyleSheet.create({
  ...
  name: { color: '#FFFFFF', fontSize: 38, fontWeight: '900', letterSpacing: -1.2 },
  metadata: { marginTop: 5, color: '#C0B8CA', fontSize: 14 },
  actions: { marginTop: 18, flexDirection: 'row', gap: 10 },
  iconButton: { width: 50, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 999, borderWidth: StyleSheet.hairlineWidth },
