...
export default function ActionSheetScreen() {
  ...
  const params = useLocalSearchParams<{ type?: string; id?: string; title?: string; subtitle?: string; image?: string; artistId?: string; source?: string; coverImages?: string }>();
  ...
  const source = params.source === 'audius' || params.source === 'crimson' ? params.source : undefined;
  const artistId = String(params.artistId || '');
  const routeCoverImages = useMemo(() => parseCoverImages(params.coverImages), [params.coverImages]);
  const [selected, setSelected] = useState(false);
  const [working, setWorking] = useState(false);
  const [showPlaylists, setShowPlaylists] = useState(false);
  const [playlists, setPlaylists] = useState<CrimsonPlaylist[]>([]);
  const [playlistSelections, setPlaylistSelections] = useState<Record<string, boolean>>({});
  const [song, setSong] = useState<CrimsonSong | null>(null);
  const [playlistArtwork, setPlaylistArtwork] = useState<Pick<CrimsonPlaylist, 'coverImages' | 'image' | 'imageSmall' | 'title'> | null>(null);
  ...
  const toggleCollection = async () => {
    ...
    setWorking(true);
    try {
      setSelected(await toggleUserCollectionItem(user.uid, collectionField, id, undefined, source));
      requestLibraryRefresh();
    }
    catch { Alert.alert('Could not update your library', 'Please try again.'); }
  ...
  const openSongArtist = async () => {
    ...
    router.dismiss();
    setTimeout(() => router.push(artistHref(targetArtistId)), reduceMotion ? 0 : 120);
  };
  ...
  const playCollection = async () => {
    ...
    try {
      ...
      router.dismiss();
    } catch { Alert.alert('Nothing to play', 'This collection does not have any available songs.'); }
    finally { setWorking(false); }
  };

  const openDetail = () => {
    router.dismiss();
    setTimeout(() => router.push(type === 'artist' ? artistHref(id) : playlistHref(id, false, source, title)), reduceMotion ? 0 : 120);
  };
  ...
  const openPlaylistPicker = async () => {
    ...
    try {
      ...
      setPlaylistSelections(Object.fromEntries(library.playlists.map((playlist) => [playlist.id, playlist.songs.includes(id)])));
    } catch { Alert.alert('Could not load playlists', 'Please try again.'); }
  };

  const openCreatePlaylist = () => {
    router.dismiss();
    setTimeout(() => {
      router.navigate(`/(app)/(library)/create-playlist?trackId=${encodeURIComponent(id)}` as Href);
    }, reduceMotion ? 0 : 120);
  };
  ...
  const togglePlaylistSong = async (playlist: CrimsonPlaylist) => {
    ...
    try {
      ...
      setPlaylistSelections((current) => ({ ...current, [playlist.id]: included }));
      requestLibraryRefresh();
    } catch { Alert.alert('Could not update the playlist', 'Please try again.'); }
    finally { setWorking(false); }
  };

  const showOfflineError = (error: unknown) => {
    const message = error instanceof DownloadError
      ? error.message
  ...
  const makePlaylistAvailableOffline = async () => {
    ...
    try {
      ...
      router.dismiss();
      void downloads.downloadSongs(detail.songs, 'manual', `playlist:${detail.playlist.id}`).then((result) => {
        if (result.failed) {
          Alert.alert(
  ...
  const primaryLabel = type === 'artist'
    ? selected ? 'Unfollow artist' : 'Follow artist'
    : type === 'playlist'
      ? selected ? 'Remove from liked playlists' : 'Like playlist'
      : selected ? 'Remove from favorites' : 'Add to favorites';
  const typeIcon: SFSymbol = type === 'artist' ? 'person.crop.circle.fill' : type === 'playlist' ? 'rectangle.stack.fill' : 'music.note';
      ...
      {showPlaylists ? (
        <View style={styles.flex}>
          <View style={styles.pickerHeader}>
            <Pressable onPress={() => setShowPlaylists(false)} style={styles.headerButton}><SymbolView name="chevron.left" size={17} tintColor={colors.text} /><Text style={[styles.headerButtonText, { color: colors.secondaryText }]}>Back</Text></Pressable>
            <Text style={[styles.pickerTitle, { color: colors.text }]}>Add to playlist</Text>
            <Pressable onPress={() => router.dismiss()} style={styles.headerButton}><Text style={[styles.doneText, { color: colors.accent }]}>Done</Text></Pressable>
          </View>
          <FlatList
            ...
            ListHeaderComponent={<ActionRow icon="plus.circle.fill" label="Create new playlist" onPress={openCreatePlaylist} />}
            ListEmptyComponent={<Text style={[styles.empty, { color: colors.secondaryText }]}>You don&apos;t have a playlist yet.</Text>}
        ...
        <View style={styles.actions}>
          <View style={styles.itemHeader}>
            <View style={[styles.itemArtwork, type === 'artist' && styles.roundArtwork]}>
              {type === 'playlist' ? (
                <PlaylistCover
                  borderRadius={17}
                  playlist={playlistArtwork || { coverImages: routeCoverImages, image, imageSmall: image, title }}
                  showPlayingIndicator={false}
                  style={StyleSheet.absoluteFill}
                />
              ) : image ? (
                <Image cachePolicy="memory-disk" contentFit="cover" source={{ uri: image }} style={StyleSheet.absoluteFill} />
              ) : (
                <SymbolView name={typeIcon} size={27} tintColor={colors.text} />
              )}
            </View>
            <View style={styles.itemCopy}><Text numberOfLines={1} style={[styles.itemTitle, { color: colors.text }]}>{title}</Text><Text numberOfLines={1} style={[styles.itemSubtitle, { color: colors.secondaryText }]}>{subtitle}</Text></View>
          </View>
          <View style={styles.separator} />
          <ActionRow disabled={working} icon={selected ? 'heart.fill' : 'heart'} label={primaryLabel} selected={selected} onPress={() => void toggleCollection()} />
          <ActionRow icon={type === 'song' ? 'person' : 'play.fill'} label={type === 'song' ? `See more from ${subtitle}` : type === 'artist' ? 'Play artist' : 'Play playlist'} onPress={type === 'song' ? () => void openSongArtist() : () => void playCollection()} />
          {type !== 'song' ? <ActionRow icon="arrow.up.right" label={type === 'artist' ? 'Open artist' : 'Open playlist'} onPress={openDetail} /> : null}
          {type === 'song' ? <ActionRow icon="plus" label="Add to playlist" onPress={() => void openPlaylistPicker()} /> : null}
          {type === 'song' ? (
            <ActionRow
          ...
          {type === 'playlist' ? <ActionRow disabled={working} icon="icloud.and.arrow.down" label="Make Playlist Available Offline" onPress={() => void makePlaylistAvailableOffline()} /> : null}
        </View>
      )}
    </View>
  );
...
function statusForActionSheet(
...
): { disabled?: boolean; icon: SFSymbol; label: string; loading?: boolean; muted?: boolean; selected?: boolean } {
  ...
  if (downloaded) return { icon: 'checkmark.circle.fill', label: 'Remove from Offline', selected: true };
  if (status.state === 'error') return { icon: 'exclamationmark.triangle', label: 'Couldn’t Save Offline' };
...
const ActionRow = memo(function ActionRow({ disabled, icon, label, loading, muted, onPress, selected }: { disabled?: boolean; icon: SFSymbol; label: string; loading?: boolean; muted?: boolean; onPress: () => void; selected?: boolean }) {
  const { colors } = useAppSettings();
  const contentColor = selected ? colors.accent : muted ? colors.mutedText : colors.text;
  return (
    <Pressable disabled={disabled} onPress={onPress} style={({ pressed }) => [styles.row, pressed && styles.rowPressed, disabled && styles.disabled]}>
      {loading
        ? <ActivityIndicator color={colors.accent} size="small" style={styles.rowIcon} />
        : <SymbolView name={icon} size={23} style={styles.rowIcon} tintColor={contentColor} />}
      <Text numberOfLines={1} style={[styles.rowLabel, { color: contentColor }]}>{label}</Text>
...
const PlaylistPickerRow = memo(function PlaylistPickerRow({
  ...
  return (
    <Pressable
      ...
      />
      <Text numberOfLines={1} style={[styles.rowLabel, { color: contentColor }]}>{playlist.title}</Text>
      <SymbolView
...
const styles = StyleSheet.create({
  ...
  flex: { flex: 1 },
  actions: { flex: 1, paddingTop: 8 },
  itemHeader: { minHeight: 108, flexDirection: 'row', alignItems: 'center', gap: 15, paddingHorizontal: 21 },
  itemArtwork: { width: 64, height: 64, overflow: 'hidden', alignItems: 'center', justifyContent: 'center', borderRadius: 17, backgroundColor: 'rgba(105,58,145,0.62)' },
  ...
  separator: { height: StyleSheet.hairlineWidth, backgroundColor: 'rgba(231,222,241,0.16)' },
  ...
  pickerHeader: { height: 62, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: 'rgba(231,222,241,0.16)' },
  headerButton: { minWidth: 68, height: 44, flexDirection: 'row', alignItems: 'center', gap: 4 },
  headerButtonText: { color: '#D8C5F7', fontSize: 15 },
  doneText: { color: '#C28EFF', fontSize: 15, fontWeight: '700', marginLeft: 'auto' },
