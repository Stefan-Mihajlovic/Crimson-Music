...
import { ActivityIndicator, Pressable, StyleSheet, Text, useWindowDimensions, View, ViewStyle } from 'react-native';
import Animated, {
  AnimatedStyle,
  cancelAnimation,
  Easing,
...
export function getPlayerArtworkLayout(width: number, height: number, safeTop: number, safeBottom = 0) {
  ...
  const verticalSize = height
    ...
    - 354;
  const size = Math.min(horizontalSize, Math.max(248, verticalSize));
  return {
    size,
    x: (width - size) / 2,
...
export function PlayerContent({
  ...
  const openLyrics = () => router.push('/player-details?tab=lyrics');
    ...
    <View style={styles.screen}>
      <View style={[styles.content, { paddingTop: Math.max(insets.top, 18), paddingBottom: Math.max(insets.bottom, 18) }]}>
        ...
        <Animated.View style={[styles.playerBody, bodyAnimatedStyle]}>
          <View style={[styles.artworkArea, { height: artworkSize + 54 }]}>
          ...
          <View style={styles.songLine}>
            <View style={styles.songCopy}>
              ...
              <Text numberOfLines={1} style={styles.artist}>{currentSong.creator}</Text>
            </View>
            <View style={styles.songActions}>
              <BouncyPressable
                ...
                <SymbolView name="quote.bubble" size={21} style={styles.songActionSymbol} tintColor={playerPalette.text} weight="semibold" />
              </BouncyPressable>
              <BouncyPressable
  ...
  return (
    <>
      ...
      <View style={styles.transportGlass}>
        ...
        <BouncyPressable accessibilityLabel={status.isBuffering ? 'Loading song' : status.playing ? 'Pause' : 'Play'} accessibilityRole="button" contentStyle={styles.playButtonContent} disabled={status.isBuffering} hitSlop={6} onPress={togglePlay} pressedScale={0.8} style={styles.playButton}>
          {status.isBuffering
...
const styles = StyleSheet.create({
  ...
  title: { color: '#F3EEFF', fontSize: 24, lineHeight: 29, fontWeight: '800', letterSpacing: -0.6 },
  ...
  seekTouchArea: { height: 26, justifyContent: 'center' },
  seekTrack: { height: 4, overflow: 'hidden', borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.14)' },
  seekFill: { height: 4, borderRadius: 2, backgroundColor: '#A66BFF' },
  times: { marginTop: -2, flexDirection: 'row', justifyContent: 'space-between' },
  time: { color: 'rgba(255,255,255,0.70)', fontSize: 11, fontVariant: ['tabular-nums'] },
