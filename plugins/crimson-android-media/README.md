...
`with-android-media-controls.js` extends the installed **expo-audio 57.0.4** sources during Android prebuild. `CrimsonRemoteCommands.kt` and `CrimsonArtworkPalette.kt` are the canonical added sources. Generated Android and dependency directories remain ignored.

`package.json` sets `expo.autolinking.android.buildFromSource` to `["expo-audio"]`. This is required: Expo 57 otherwise links its prebuilt audio AAR and ignores local source patches. The plugin validates this setting; all other Expo modules keep their prebuilt optimization.
